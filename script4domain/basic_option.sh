      ...
      "interfaces": [
        {
          ...
          "description": "",
          "deviceIp": "31.31.31.1",
          "deviceAddress": "31.31.31.1",
          "deviceName": "31.31.31.1"
        },
        {
          "id": 93454338,
          "name": "ifindex 1",
          "ifIndex": 1,
          "description": "",
          "deviceIp": "172.24.2.137",
          "deviceAddress": "172.24.2.137",
          "deviceName": "172.24.2.137"
        }
      ]
    }
EOF
...
jsoncurl POST http://localhost:9092/api/v1/config/collection/domain/10/Sites @newsite$$.json | egrep "\"affectedRecords\":1"
if [ $? != 0 ] 
then
...
#The parameter include: server_address server_name, if server_name is blank,use server_address as server_name
#example: addserver 172.1.1.1 server1
...
jsoncurl POST http://localhost:9092/api/v1/config/collection/domain/10/Servers @newserver$$.json | egrep "\"affectedRecords\":1"
if [ $? != 0 ] 
then
		echo "Fail to add server:$1" >> servererror.txt; 
		echo -1
		return 1
else
		printf "Success to add server $1"
fi
...
#The parameter include: appname appport
#example: addapp rokftp 2121
...
jsoncurl POST http://localhost:9092/api/v1/config/collection/domain/10/Applications @newapp$$.json | egrep "\"affectedRecords\":1"
if [ $? != 0 ] 
then
		echo "Fail to add application:$1" >> applicationerror.txt; 
		echo -1
		return 1
else
		printf "Success to add application $1"
fi
